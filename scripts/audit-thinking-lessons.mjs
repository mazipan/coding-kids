/**
 * Content audit for the thinking path. Run with `bun scripts/audit-thinking-lessons.mjs`.
 *
...
const seen = new Set();
for (const l of THINKING_LESSONS) {
  if (seen.has(l.id)) fail(`duplicate lesson id: ${l.id}`);
  seen.add(l.id);
  if (l.id !== `${l.worldId}-${l.number}`) fail(`id does not match worldId-number: ${l.id}`);
  if (!(l.xpReward > 0)) fail(`${l.id}: xpReward must be positive`);
}

for (const world of THINKING_WORLDS) {
  const lessons = THINKING_LESSONS.filter((l) => l.worldId === world.id).sort(
    (a, b) => a.number - b.number,
  );
  if (lessons.length !== world.lessonCount) {
    fail(`${world.id}: lessonCount is ${world.lessonCount} but ${lessons.length} lessons exist`);
  }
  lessons.forEach((l, i) => {
    // INV-L1 — sequential unlock only works on a contiguous run starting at 0.
    if (l.number !== i) fail(`${world.id}: lesson numbers are not contiguous at ${l.id}`);
  });
  ...
  for (const l of lessons) {
    ...
    if (run >= 3)
      fail(`${world.id}: ${run} consecutive true-false puzzles answer ${last} (ends at ${l.id})`);
  }
  ...
  if (tierTwo.length > 0) {
    ...
    if (avg(tierTwo) <= avg(tierOne))
      fail(`${world.id}: tier two does not reward more XP than tier one`);
  }
}
...
const bothLangs = (id, field, value) => {
  if (!value || typeof value.en !== 'string' || !value.en.trim())
    fail(`${id}: ${field} has no English text (INV-I1)`);
  if (!value || typeof value.id !== 'string' || !value.id.trim())
    fail(`${id}: ${field} has no Indonesian text (INV-I1)`);
};
...
for (const l of THINKING_LESSONS) {
  ...
  const uniqueOptions = (opts) => {
    if (new Set(opts).size !== opts.length) fail(`${l.id}: options contain a duplicate`);
    if (opts.length < 2) fail(`${l.id}: needs at least 2 options (INV-Q4)`);
  };

  switch (p.type) {
    case 'pattern':
      uniqueOptions(p.options);
      if (!p.options.includes(p.answer)) fail(`${l.id}: answer is not one of the options`);
      if (p.items[p.blankIndex] !== '?') fail(`${l.id}: blankIndex does not point at the '?'`);
      break;
    case 'math':
    ...
    case 'if-then': {
      ...
      uniqueOptions(ids);
      if (!ids.includes(p.answerId)) fail(`${l.id}: answerId is not one of the options`);
      p.options.forEach((o) => {
    ...
    case 'sort':
      if (p.items.length !== p.answer.length)
        fail(`${l.id}: sort answer length does not match items`);
      if ([...p.items].sort().join() !== [...p.answer].sort().join())
        fail(`${l.id}: sort answer is not a permutation of items`);
      uniqueOptions(p.items);
      if (p.items.join() === p.answer.join())
        fail(`${l.id}: sort items are already in the answer order`);
    ...
    case 'fill-in':
      ...
      if (!/^-?[\d.,/]+$/.test(p.answer))
        fail(`${l.id}: fill-in answer "${p.answer}" is not language-neutral (INV-C2)`);
      break;
    case 'match':
    ...
    case 'abstraction': {
      ...
      if (p.correctIds.length === 0) fail(`${l.id}: no correct items`);
      if (p.correctIds.some((c) => !ids.includes(c)))
        fail(`${l.id}: a correctId is not among the items`);
      if (p.subtype === 'odd-one-out' && p.correctIds.length !== 1)
        fail(`${l.id}: odd-one-out must have exactly 1 answer`);
      if (p.correctIds.length === ids.length)
        fail(`${l.id}: every item is correct, so there is nothing to decide`);
      break;
    }
    case 'spatial': {
      bothLangs(l.id, 'question', p.question);
      ...
      const checkGrid = (g, what) => {
        if (g.length !== rows) fail(`${l.id}: ${what} has ${g.length} rows, figure has ${rows}`);
        g.forEach((row) => {
          if (row.length !== cols) fail(`${l.id}: ${what} row "${row}" is not ${cols} wide`);
          if (/[^#o.]/.test(row)) fail(`${l.id}: ${what} row "${row}" has a character outside #o.`);
        });
      };
      ...
      if (new Set(drawings).size !== drawings.length)
        fail(`${l.id}: two options draw the same grid`);
      break;
    }
    case 'multi-step': {
      bothLangs(l.id, 'intro', p.intro);
      if (p.steps.length < 2) fail(`${l.id}: a chain needs at least 2 steps`);
      uniqueOptions(p.steps.map((s) => s.id));
      p.steps.forEach((s) => {
        ...
        if (!ids.includes(s.answerId))
          fail(`${l.id}: step ${s.id} answerId is not one of its options`);
      });
      break;
    }
    case 'grid-select': {
      ...
      p.cells.forEach((row, r) => {
        if (row.length !== cols)
          fail(`${l.id}: row ${r} has ${row.length} cells, row 0 has ${cols}`);
      });
      if (p.answer.length === 0) fail(`${l.id}: grid-select has no answer cells`);
      if (new Set(p.answer).size !== p.answer.length)
        fail(`${l.id}: grid-select answer repeats a cell`);
      p.answer.forEach((key) => {
        const [r, c] = key.split('-').map(Number);
        if (!(r >= 0 && r < p.cells.length && c >= 0 && c < cols))
          fail(`${l.id}: answer cell ${key} is outside the grid`);
      });
      break;
    }
    default:
      fail(`${l.id}: unknown puzzle type ${p.type}`);
  }
}

const byType = {};
for (const l of THINKING_LESSONS) byType[l.puzzle.type] = (byType[l.puzzle.type] ?? 0) + 1;

console.log(`${THINKING_LESSONS.length} lessons across ${THINKING_WORLDS.length} worlds`);
console.log(
